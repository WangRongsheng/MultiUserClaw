...
参考标准的claude的skill私有市场，可以添加企业内的skill市场
...
python deploy_docker.py --build-only   构建nanobot user的 docker镜像
...
本地开发环境已启动
       ...
       Nanobot Web  http://127.0.0.1:18080  (PID 37944)
  Platform Gateway  http://127.0.0.1:8080  (已有实例)
      Frontend Dev  http://127.0.0.1:3080  (已有实例)
```

python deploy_docker.py --host=192.168.1.xxx 一键部署到服务器，可以仔细查看下配置信息，host为本机或者服务器的ip地址

python deploy_docker.py --rebuild gateway 可以重新build镜像，如果修改了platform的代码

python deploy_docker.py --rebuild frontend 可以重新build镜像，如果修改前端的代码
